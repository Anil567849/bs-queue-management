...
export async function POST(request: Request){
    ...
    if (myFile && myFile.length > 0){
            ...
            for(let i = 0; i < myFile.length; i++){
                if(myFile[i].type == 'text/plain'){
                    try {
                        
                        const text: string = await FileManager.getText(myFile[i]);
                        // console.log('File content:', text);
                        
                        const marks: number = await FileManager.getMarksFromText(text);
                ...
                }else if(myFile[i].type == 'image/png'){
                    try {
                        
                        const text: string = await FileManager.getBase64Encoded(myFile[i]);
                        // console.log('File content:', text);
                        
                        const marks: number = await FileManager.getMarksFromVision(text);
                    ...
                    return Response.json({ message: 'error reading image' }, { status: 500 });
                }
            }
