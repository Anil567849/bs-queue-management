...
function Classes({ params }: { params: { semester: string } }) {
  ...
  return (
    <>
      <main className="flex min-h-[90vh] flex-col items-center justify-start">
        <div className="w-[70vw] py-9 text-center">
        <h1 className="pb-5 text-5xl font-bold leading-[120%]">Select a <span className="text-blue-800">Class</span></h1>
        </div>
        ...
        <div className="w-[30vw] pb-5 text-center">
              { semester.length != 0 && semester.map((item: any, ind: number) => {
                return <Link 
                key={ind}
                href={`/admin/${params.semester}/${item.class}`}
